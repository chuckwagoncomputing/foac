# Keep track of who has three pseudonyms, with a 50% chance of remembering
class StrategythreePseudonymsMaybe
 ...
 def remember(calledName, calledTeam, calledPosition, calledPseudonym, callerName)
  ...
  # if there are less than three items in my memory and I was not it, and a coin toss says to
  if @memory.length < 3 and callerName != @player.name and [0,1].sample == 0
 ...
 def think
  ...
  @memory.reject{|key, value| value == @strategy.lastPseudonym }.each do |mem|
   ...
   # if they are on my team and they are on the couch, better a random number, with theirs rejected. If I am able to call to the couch, this will have already been caught.
    ...
    puts "else called in strategy threePseudonymsMaybe.rb"
    puts @player.team
